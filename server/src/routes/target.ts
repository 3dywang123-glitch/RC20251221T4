...
router.get('/', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const result = await pool.query(
      'SELECT * FROM target_profiles WHERE user_id = $1 ORDER BY created_at DESC',
      [req.userId]
    );
...
router.get('/:id', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM target_profiles WHERE id = $1 AND user_id = $2',
      [id, req.userId]
    );
    ...
    // Get related data
    const personalityReport = await pool.query(
      'SELECT * FROM personality_reports WHERE target_id = $1 ORDER BY created_at DESC LIMIT 1',
      [id]
    );

    const socialAnalysis = await pool.query(
      'SELECT * FROM social_analysis_results WHERE target_id = $1 ORDER BY created_at DESC',
      [id]
    );

    const postAnalysis = await pool.query(
      'SELECT * FROM social_post_analysis WHERE target_id = $1 ORDER BY created_at DESC',
      [id]
    );

    const relationshipReports = await pool.query(
      'SELECT * FROM relationship_reports WHERE target_id = $1 ORDER BY created_at DESC',
      [id]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO target_profiles 
       ...
       RETURNING *`,
      [req.userId, name, occupation || '', bio || '', age || '', avatarB64 || '', additionalImages || [], socialLinks || '', generalSummary || '']
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE target_profiles SET
       ...
       RETURNING *`,
      [name, occupation, bio, age, avatarB64, additionalImages, socialLinks, generalSummary, id, req.userId]
    );
...
router.delete('/:id', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await pool.query(
      'DELETE FROM target_profiles WHERE id = $1 AND user_id = $2 RETURNING id',
      [id, req.userId]
    );
...
router.post('/:id/personality', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
router.post('/:id/social-analysis', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
// Save post analysis
router.post('/:id/post-analysis', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const { content, images, analysis, suggestedReplies, tags, timestamp } = req.body;

    await pool.query(
      `INSERT INTO social_post_analysis 
       (target_id, content, images, analysis, suggested_replies, tags, timestamp)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [id, content, images, analysis, suggestedReplies, tags, timestamp]
    );
...
router.post('/:id/relationship-report', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const report = req.body;
