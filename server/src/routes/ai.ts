...
// Analyze personality
router.post('/analyze-personality', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (additionalImages) images.push(...additionalImages.slice(0, 2));
...
router.post('/analyze-social', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.post('/relationship-report', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
